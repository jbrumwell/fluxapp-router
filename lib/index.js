  ...
  routes = fluxApp.createStore(STORE_NAME, {
    ...
    onRouteChange : function onRouteChange(payload) {
      ...
      if (payload.changeBy) {
        ...
        var route = routeStore.getRoute(payload.path, { method : payload.params.method });
...
function initActions() {
  actions = fluxApp.createActions(ACTION_PREFIX, {
    change : function routeChangeAction(path, params, action) {
      ...
      if (! params.changeBy) {
        var route = routeStore.getRoute(action.path, { method : params.method });
        if (! route) {
...
module.exports = {
  ...
  addRoute : routeStore.addRoute.bind(routeStore),
  getRoute : routeStore.getRoute.bind(routeStore)
};

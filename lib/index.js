...
function initActions() {
  fluxApp.createActions(ACTION_PREFIX, {
...
function forward() {
  return fluxApp.getAction(ACTION_PREFIX, 'change')(undefined, { changeBy : 1 });
}
...
function init() {
  ...
  window.addEventListener('popstate', function(e) {
    if (e.state) {
      ...
      if (offset !== null) {
        fluxApp.getAction(ACTION_PREFIX, 'change')(undefined, { changeBy : offset });
      } else {

        // Sanitize the state
        delete e.state.changeBy;
        fluxApp.getAction(ACTION_PREFIX, 'change')(undefined, undefined, e.state);
      }
    } else {
      fluxApp.getAction(ACTION_PREFIX, 'change')(window.location.pathname, { force : true });
    }
  }, false);
...
function go(path, params, action) {
  var changeAction = fluxApp.getAction(ACTION_PREFIX, 'change');
...
function getActions() {
  return fluxApp.getActions(ACTION_PREFIX);
}
